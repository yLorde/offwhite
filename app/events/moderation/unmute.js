...
module.exports = {
	...
	async execute(message,client) {
        ...
        if (message.content.startsWith(`${prefix}desmutarcall`)) {
            ...
            if( args.length<=1){
                await message.channel.send({embeds: [new MessageEmbed()
                    ...
                    Exemplo: ${prefix}desmutarcall 22222222 Perdoado`)
                ]})
                return;
            }
            checkPerm(staffId,"unmute").then(res=>{
                 if(res){
                    isPunish(args[1].replace("<","").replace("@","").replace(">",""),'mute').then(res=>{
                        if(!res){
                            ...
                            client.users.fetch(args[1].replace("<","").replace("@","").replace(">",""), false).then((user) => {
                                ...
                                removeRole(roleMute,args[1].replace("<","").replace("@","").replace(">",""))
